...
// if(isset($_POST['create_report'])) {}
    ...
    $sql = "SELECT * from `ordertbl` WHERE `Display` = '0'";
...
$pdf->SetSubject('FTMS');
$pdf->SetKeywords('TCPDF, PDF, FTMS, Order, Report');
...
if($type == '2'){
    ...
    $pdf -> Cell(189, 5 , 'Order in Progress', 0, 0, 'C');
    $pdf->Ln(15);
    ...
    $pdf -> Cell(30, 7 , 'Delivery Date', 1, 0, 'C', 1);
    $pdf -> Cell(23, 7 , 'Price', 1, 0, 'C', 1);
    $pdf -> Cell(26, 7 , 'Status', 1, 0, 'C', 1);
    $pdf -> Cell(35, 7 , 'Description', 1, 0, 'C', 1);
    // ordid, cusid, cusName, styleId, ordDate, fitonDate, deliveryDate, ordPrice, ordDiscount, measId, ordProgress, ordDescription, 
    $i = 1;
    $max = 20;
    ...
    while( $row = mysqli_fetch_array($res) ){
        ...
        $styleId = $row['styleId'];
        $ordDate = $row['ordDate'];
        $fitonDate = $row['fitonDate'];
        $deliveryDate = $row['deliveryDate'];
        $Price = $row['ordPrice'];
        $Discount = $row['ordDiscount'];
        $measId = $row['measId'];
        $progress = $row['ordProgress'];
        $Description = $row['ordDescription'];
        $status;
        ...
        $pdf -> Cell(35, 7 , $cusName, 1, 0, 'L', 0);
        $pdf -> Cell(30, 7 , $ordDate, 1, 0, 'C', 0);
        $pdf -> Cell(30, 7 , $deliveryDate, 1, 0, 'C', 0);
        $pdf -> Cell(23, 7 , "Rs.".$Price, 1, 0, 'R', 0);
        $pdf -> Cell(26, 7 , $status, 1, 0, 'L', 0);
        $pdf -> Cell(35, 7 , $Description, 1, 0, 'L', 0);
    }
}else if($type == '3'){
    $pdf->Ln(35);
    $pdf->SetFont('dejavusans', '', 12, '', true);
    $pdf -> Cell(189, 5 , 'Finished Orders', 0, 0, 'C');
    $pdf->Ln(15);
    $pdf->SetFont('dejavusans', '', 11, '', true);
    $pdf -> SetFillColor(224,225,225);
    $pdf -> Cell(10, 7 , 'Id', 1, 0, 'C', 1);
    ...
    $pdf->Ln(35);
    $pdf->SetFont('dejavusans', '', 12, '', true);
    $pdf -> Cell(189, 5 , 'Order Detailed Report', 0, 0, 'C');
    $pdf->Ln(15);
    $pdf->SetFont('dejavusans', '', 11, '', true);
    $pdf -> SetFillColor(224,225,225);
    $pdf -> Cell(10, 7 , 'Id', 1, 0, 'C', 1);
