...
// if(isset($_POST['create_report'])) {}
    ...
    $payid = $_POST['payid_inv'];
    $invid_inv = $_POST['invid_inv'];

    $sql = "SELECT * from `payment` WHERE `payid`=$payid AND `Display` = '0'";
...
while( $row = mysqli_fetch_array($res) ){
    ...
    $pdf->Ln(7);
    $pdf -> Cell(193, 7 , 'Paid', 1, 0, 'R', 0);
    $pdf -> Cell(75, 7 , 'Rs.'.$pamount, 1, 0, 'C', 1);
    $pdf->Ln(7);
    $pdf -> Cell(193, 7 , 'Balance', 1, 0, 'R', 0);
    $pdf -> Cell(75, 7 , 'Rs.'.$pbalance, 1, 0, 'C', 1);
