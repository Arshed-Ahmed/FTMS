...
// if(isset($_POST['create_report'])) {}
    ...
    $empid = $_POST['report_type'];

    $sql = "SELECT * from `jobcard` WHERE `tid`=".$empid   ;
...
$sql2 = "SELECT * from `employee` WHERE `tid`=".$empid." AND `Display` = '0'";
...
$pdf -> Cell(189, 5 , 'Orders for the Employee '.$empname, 0, 0, 'C');
$pdf->Ln(15);
$pdf->SetFont('dejavusans', '', 11, '', true);
$pdf -> SetFillColor(224,225,225);
$pdf -> Cell(20, 7 , 'Order Id', 1, 0, 'C', 1);
$pdf -> Cell(50, 7 , 'Order Date', 1, 0, 'C', 1);
$pdf -> Cell(90, 7 , 'Desc', 1, 0, 'C', 1);
$pdf -> Cell(29, 7 , 'Fit on Date', 1, 0, 'C', 1);
...
while( $row = mysqli_fetch_array($res) ){
    ...
    $sql1 = "SELECT * from `ordertbl` WHERE `ordid`=".$ordid." AND `Display` = '0'";
    $res1 = mysqli_query($conn, $sql1);
    
    $row1 = mysqli_fetch_array($res1) ;

    $id = $row1['ordid'];
    $ordDate = $row1['ordDate'];
    $Desc = $row1['ordDescription'];
    $fitDate = $row1['fitonDate'];
    ...
    $pdf->Ln(7);
    $pdf -> Cell(20, 7 , $id, 1, 0, 'C', 0);
    $pdf -> Cell(50, 7 , $ordDate , 1, 0, 'C', 0);
    $pdf -> Cell(90, 7 ,$Desc, 1, 0, 'C', 0);
    $pdf -> Cell(29, 7 , $fitDate, 1, 0, 'C', 0);
